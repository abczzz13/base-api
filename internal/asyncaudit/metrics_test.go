...
func TestNewMetricsValidation(t *testing.T) {
	metrics, err := NewMetrics(nil, testMetricsConfig())
	if metrics != nil {
...
func TestNewMetricsSupportsAlreadyRegisteredCollectors(t *testing.T) {
	...
	metricsA, err := NewMetrics(registry, testMetricsConfig())
	if err != nil {
		t.Fatalf("NewMetrics(first) returned error: %v", err)
	}

	metricsB, err := NewMetrics(registry, testMetricsConfig())
	if err != nil {
		t.Fatalf("NewMetrics(second) returned error: %v", err)
	}

	metricsA.observeRecordResult("", ResultEnqueued)
	metricsB.observeQueueDepth(3)
	metricsB.observeWriteDuration("svc", 25*time.Millisecond, WriteResultSuccess)

	if got := testutil.ToFloat64(metricsA.recordsTotal.WithLabelValues("unknown", ResultEnqueued)); got != 1 {
