...
func TestNewMetricsSupportsAlreadyRegisteredCollectors(t *testing.T) {
	...
	metricsA.observeRecordResult("", outboundAuditResultEnqueued)
	metricsB.observeQueueDepth(3)
	metricsB.observeWriteDuration("billing", 25*time.Millisecond, outboundAuditWriteResultSuccess)

	if got := testutil.ToFloat64(metricsA.recordsTotal.WithLabelValues(outboundAuditClientUnknown, outboundAuditResultEnqueued)); got != 1 {
