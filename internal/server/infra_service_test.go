...
func TestInfraServiceGetMetrics(t *testing.T) {
	tests := []struct {
		name    string
		wantErr *infraoas.DefaultErrorStatusCode
	}{
		{
			name: "returns explicit misconfiguration response",
			wantErr: &infraoas.DefaultErrorStatusCode{
				StatusCode: 500,
				Response: infraoas.ErrorResponse{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResp, err := svc.GetMetrics(context.Background())
			if gotResp != nil {
				t.Fatalf("GetMetrics response mismatch: want nil, got %T", gotResp)
			}

			var gotErr *infraoas.DefaultErrorStatusCode
			if !errors.As(err, &gotErr) {
				t.Fatalf("GetMetrics error type mismatch: got %T (%v)", err, err)
			}
			if diff := cmp.Diff(tt.wantErr, gotErr); diff != "" {
				t.Fatalf("GetMetrics error mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
