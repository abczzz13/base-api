...
func TestDefaultReadinessCheckers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			checkers := defaultReadinessCheckers(tt.database)
			...
			if tt.wantName != "" {
				if got := readinessCheckerLogName(checkers[0], 0); got != tt.wantName {
...
func TestWithReadinessCheckerNameNilChecker(t *testing.T) {
	checker := withReadinessCheckerName("database", nil)
	if checker == nil {
		t.Fatal("withReadinessCheckerName returned nil checker")
	}

	err := checker.CheckReadiness(context.Background())
	if !errors.Is(err, errNilReadinessChecker) {
		t.Fatalf("CheckReadiness error mismatch: want %v, got %v", errNilReadinessChecker, err)
	}

	if got := readinessCheckerLogName(checker, 0); got != "database" {
